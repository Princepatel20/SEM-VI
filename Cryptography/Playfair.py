...
#     plaintextpairs = []
#     ciphetextpairs = []
#     # Rule 1: if both letters are same or only one left add "X" after first letter
#     i = 0
...
#         if applied_rule:
#             continue
#         # Rule 3 :If letters are in same column, replace them with immediate below letter
#         for j in range(5):
...
#         # the other pair of corners of the rectangle defined by the original pairs
#         i0 = 0
#         i1 = 0
#         j0 = 0
#         j1 = 0
...
def genKeyMat(key):
    ...
    key_matrix = ['' for i in range(6)] 
    i = 0
    j = 0
    for c in key:
        if c in alphanumeric:
            key_matrix[i] += c
            alphanumeric = alphanumeric.replace(c, '')
            j += 1
        if j > 5: 
            i += 1
            j = 0
    ...
    for pair in plaintextpairs:
        ...
        for row in key_matrix:
            if pair[0] in row and pair[1] in row:
                j0 = row.find(pair[0])
                j1 = row.find(pair[1])
                ciphetextpair = row[(j0 + 1) % 6] + row[(j1 + 1) % 6] 
                ciphetextpairs.append(ciphetextpair)
                applied_rule = True
                if applied_rule:
                    break
        if applied_rule:
            continue
        for j in range(6): 
            col = "".join([key_matrix[i][j] for i in range(6)])
            if pair[0] in col and pair[1] in col:
                i0 = col.find(pair[0])
                i1 = col.find(pair[1])
                ciphetextpair = col[(i0 + 1) % 6] + col[(i1 + 1) % 6] 
                ciphetextpairs.append(ciphetextpair)
                applied_rule = True
                if applied_rule:
                    break
    ...
    for pair in ciphetextpairs:
        ...
        for row in key_matrix:
            if pair[0] in row and pair[1] in row:
                j0 = row.find(pair[0])
                j1 = row.find(pair[1])
                encryptedtextpair = row[(j0 + 5) % 6] + row[(j1 + 5) % 6] 
                encryptedtextpairs.append(encryptedtextpair)
                applied_rule = True
                if applied_rule:
                    break
        ...
        for j in range(6): 
            ...
            if pair[0] in col and pair[1] in col:
                i0 = col.find(pair[0])
                i1 = col.find(pair[1])
                encryptedtextpair = col[(i0 + 5) % 6] + col[(i1 + 5) % 6] 
                encryptedtextpairs.append(encryptedtextpair)
                applied_rule = True
                if applied_rule:
                    break
